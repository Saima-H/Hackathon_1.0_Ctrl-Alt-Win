...
-- STEP 2: safe signup profile trigger function
...
-- STEP 3: attach trigger to Supabase auth users
drop trigger if exists on_auth_user_created on auth.users;
create trigger on_auth_user_created
after insert on auth.users
for each row execute function public.handle_new_user();

-- STEP 4: remove recursive profile policy
drop policy if exists "staff can read profiles" on public.profiles;

-- STEP 5: staff can read tickets
...
using (
  exists (
    ...
    where p.id = auth.uid()
    and p.role in ('ghmc_staff', 'admin')
  )
);

-- STEP 6: staff can create/delete announcements
